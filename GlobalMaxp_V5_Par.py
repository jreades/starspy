...
class GlobalMaxp:
    ...
    def __init__(self,w,z,floor,floor_variable,
        ...
        if not self.p:
            ...
            arguments = [[curval, initial/4, self.w, self.z]]
            arguments.append([curval, initial/4, self.w, self.z])
            arguments.append([curval, initial/4, self.w, self.z])
            arguments.append([curval, initial/4, self.w, self.z])
...
def initial_solution(w, preseeds, floor_variable, floor):
        new_p=0
        solving=True
        attempts=0
        while solving and attempts<=MAX_ATTEMPTS:
            ...
            while candidates:
                ...
                while building_region:
                    # check if floor is satisfied
                    if check_floor(w, floor_variable, floor, region):
                        regions.append(region)
                        building_region=False
                    else:
                        potential=[] 
...
def check_floor(w, floor_variable, floor, region):                
...
def objective_function(w, z, solution):
        # solution is a list of lists of region ids [[1,7,2],[0,4,3],...] such
        # that the first region has areas 1,7,2 the second region 0,4,3 and so
        # on. solution does not have to be exhaustive
        wss=0
...
def pickBest(argu):
        ...
        attempts=0
        for i in range(initial):
            tmp_regions, tmp_area2region, p = initial_solution()
            if p:
                val = objective_function(w, z, tmp_regions)
