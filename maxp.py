...
class Maxp:
    ...
    def __init__(self,w,z,floor,floor_variable,
        ...
        self.seeds=seeds
        self.initial_solution() # do we need this XXX
        if not self.p:
            self.feasible = False
        else:
            self.feasible = True 
            best_val=self.objective_function() # set at start XXX
    ...
    def initial_solution(self):
        ...
        while solving and attempts<=MAX_ATTEMPTS:
            ...
            while candidates:
                ...
                while building_region:
                    ...
                    if self.check_floor(region):
                        ...
                        if potential:
                            # add a random neighbor or use some other
                            # selection criteriaXXX
                            neigID=random.randint(0,len(potential)-1) # other ways to select?
            ...
            while enclaves and encAttempts!=encCount:
                ...
                if candidates:
                    # add enclave to random region or use other
                    # criteriaXXX
