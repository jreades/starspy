...
import pysal
from pysal.region import check_contiguity
...
class GlobalMaxp:
                            ...
                            for i in possiblemoveMatrix:
                                for j in i:
                                    if not j == 0:
                                        # self.current_regions = copy.deepcopy(self.regions)
                                        # try:
                                            # self.current_regions[j[2]].remove(j[0])
                                            # self.current_regions[j[3]].append(j[0])
                                        # except:
                                            # print i
                                            # print j
                                            # print self.current_regions[j[2]]
                                            # print self.current_regions[self.area2region[j[0]]]
                                            # exit()
                                        # finally:
                                            # j[5] = self.objective_function(self.current_regions)
                                        preReg = j[2]
                                        aftReg = j[3]
    ...
    def weighted_objective_function(self, solution = None):
        # solution is a list of lists of region ids [[1,7,2],[0,4,3],...] such
        # that the first region has areas 1,7,2 the second region 0,4,3 and so
        # on. solution does not have to be exhaustive
        if not solution:
