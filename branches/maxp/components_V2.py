...
def check_contiguity_par(neighborDict,neighbors,leaver):
    """Check if contiguity is maintained if leaver is removed from neighbors
...
def check_contiguity_breadth(w,neighbors,leaver):
    """Check if contiguity is maintained if leaver is removed from neighbors
    ...
    for i in d:
        for j in d[i]:
            g.add_edge(i,j,1.0)
    cc=g.connected_components_breadth(op=gt)
    if len(cc)==1:
        neighbors.remove(leaver)
        if cc[0].nodes == set(neighbors):
            return True 
...
class Graph(object):
    ...
    def dfs(self, v, visited, threshold, op=lt, first=None):
        ...
        if first is None:
            first = v
        for i in ((n, w) for n, w in self.edges.get(v,{}).iteritems() \
                  if n not in visited):
            x,y=self.dfs(i[0],visited,threshold,op,first)
            if op(i[1], threshold):
                aux.extend(x)
                visited=visited.union(y)
        return aux, visited
    
    def bfs(self, v, aux, visited, threshold, op=lt, first=None):
        if first is None:
            first = v
        for i in ((n,w) for n,w in self.edges.get(v,{}).iteritems() if n not in aux):
            aux.append(i[0])
            visited.add(i)
        
        if len(visited) > 0:
            nextV, nextW = visited.pop()
            while not op(nextW, threshold):
                nextV, nextW = visited.pop()
            aux, visited = self.bfs(nextV, aux, visited, threshold, op, first)
        
        return aux, visited
        
    def connected_components_breadth(self, threshold=0.9, op=lt):
        nodes = set(self.nodes)
        components, visited =[], set()
        while len(nodes) > 0:
            connected = []
            ...
            connected, visited = self.bfs(first, connected, visited, threshold, op)
            connected = set(connected)
            for node in connected:
                if node in nodes:
                    nodes.remove(node)
